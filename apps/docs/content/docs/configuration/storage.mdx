...
The available options are: `brand`, `size`, `type`, `temperature`, `raid`.

- type: `string (comma separated list)`
- default: `brand,type,size,temperature,raid`

### `DASHDOT_ENABLE_STORAGE_TEMPS`

Shows the current NVMe Composite temperature in the storage widget. On Linux,
dash. reads the sensor from `/sys/class/nvme`, so the host `/sys` directory must
